...
#function for sendint at commands
...
while True:
   ...
   ###############################
    ...
    print("1-----For Each New Object Detected------------------------------------------")
    ...
    for i in range(len(scores)):
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if object_name in tracked_object_types:
                print("1a-----For Object[",i,"] Detected------------------------------------------")
   ...
   #tracking center point of bounding boxes 
    ...
    if count < 0:
        ...
        print("2-----Outside - For Each Tracking Object------------------------------------------")
        for (object_id, pt2) in tracking_objects_copy.items():
            object_exists = False
            #print(object_id)
            print("2a-----Outside - For Each Center Point for Point[",object_id,"]-- at [",pt2,"]------------------------------------------")
            for pt, i in center_points_cur_frame_copy:
                    print("2aa-----Inside - For Center Point[",i,"]-- at [",pt,"]----------------------------------------")
                    print(center_points_cur_frame_copy)
                    #if object_id == i:
                    print((pt,i))
                    distance = math.hypot(pt2[0] - pt[0], pt2[1] - pt[1])
                    print("distance is = ",distance," for object_id ",object_id)
                    #distance = math.sqrt(+pow((pt2[0] - pt[0]),2)+pow((pt2[1] - pt[1]),2))
                    print("point x current ",pt[0]," previous point x ",pt2[0]," object id ",object_id)
                    print("point y current ",pt[1]," previous point y ",pt2[1]," object id ",object_id)
                ## Update IDs position
                    if distance < 95:
                        ##for trying to update with a class
                        #tracking_objects[object_id] = TrackedObject()
                        #tracking_objects[object_id].distance = distance
                        #tracking_objects[object_id].object_type = labels[int(classes[i])]
                        #tracking_objects[object_id].location = pt
                        ...
                        if (pt,i) in center_points_cur_frame:
                            print("2ab-----Inside - REMOVING Center Point[",i,"]-- at [",pt,"]----------------------------------------")        
                        ...
                        if cv2.contourArea(c) > min_contour[object_name]:
                            ...
                            if distance2 >= 45 and distance2 <= 200:
                                ...
                                if not object_id in text_sent_id and (tracking_object_keep_alive[object_id] == KEEP_ALIVE_NUM_START) and (cur_time - last_text_time)> 60.0 :
                                    ...
                                    print(object_name,"Intruderdistcheck-- detected with motion")
